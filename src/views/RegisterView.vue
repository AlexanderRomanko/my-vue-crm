<template>
  <v-container class="fill-height">

    <v-form class="row justify-center text-center" @submit.prevent="submit">

      <v-card class="col col-lg-3 col-sm-6 mx-5 px-5">

        <h1 class="justify-center teal--text">Register page</h1>

        <v-text-field
            v-model.trim="email"
            :error-messages="emailErrors"
            label="E-mail"
            autocomplete="email"
            required
            @input="$v.email.$touch()"
            @keydown.enter="submit"
        ></v-text-field>

        <v-divider class="mt-1"></v-divider>

        <v-text-field
            v-model.trim="password"
            :error-messages="passwordErrors"
            label="Password"
            :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
            :type="show ? 'text' : 'password'"
            @click:append="show = !show"
            autocomplete="password"
            required
            @input="$v.password.$touch()"
            @keydown.enter="submit"
        ></v-text-field>

        <v-divider class="mt-1"></v-divider>

        <v-text-field
            v-model.trim="name"
            :error-messages="nameErrors"
            label="Name"
            required
            @input="$v.name.$touch()"
            @keydown.enter="submit"
        ></v-text-field>

        <v-divider class="mt-1"></v-divider>

        <v-checkbox
            v-model="checkbox"
            :error-messages="checkboxErrors"
            label="Agree with rules"
            color="success"
            required
            @change="$v.checkbox.$touch()"
        ></v-checkbox>

        <v-divider class="mt-1 mb-6"></v-divider>

        <v-btn block color="success" class="mb-5" @click="submit">Sign up
          <v-icon right>mdi-send</v-icon>
        </v-btn>

        <v-btn block color="accent" class="mb-5" @click="logInWithGoogle">
          <v-icon left>mdi-google</v-icon>Log in with Google
        </v-btn>

        <p>
          Have an account?
          <router-link to="/login" class="deep-orange--text">Log in</router-link>
        </p>

      </v-card>

    </v-form>

  </v-container>

</template>

<script>
import { required, email, minLength } from 'vuelidate/lib/validators'

export default {

  name: 'RegisterView',

  validations: {
    email: { required, email },
    password: { required, minLength: minLength(8) },
    name: {required, minLength: minLength(2) },
    checkbox: { checked: val => val }
  },

  data: () => ({
    email: '',
    password: '',
    name: '',
    checkbox: false,
    show: false
  }),

  methods: {
    async submit() {
      this.$v.$touch()
      if (this.$v.$invalid) {
        return
      }
      const formData = {
        email: this.email,
        password: this.password,
        name: this.name
      }
      try {
        await this.$store.dispatch('register', formData)
        await this.$router.push('/')
      } catch (e) {}
    },
    async logInWithGoogle() {
      try {
        await this.$store.dispatch('logInWithGoogle')
        await this.$router.push('/')
      } catch (e) {}
    }
  },

  computed: {
    emailErrors () {
      const errors = []
      if (!this.$v.email.$dirty) return errors
      !this.$v.email.email && errors.push('Must be valid e-mail')
      !this.$v.email.required && errors.push('E-mail is required')
      return errors
    },
    passwordErrors () {
      const errors = []
      if (!this.$v.password.$dirty) return errors
      !this.$v.password.minLength && errors
          .push(`Must be at least ${this.$v.password.$params.minLength.min} characters long`)
      !this.$v.password.required && errors.push('Password is required')
      return errors
    },
    nameErrors () {
      const errors = []
      if (!this.$v.name.$dirty) return errors
      !this.$v.name.minLength && errors
          .push(`Must be at least ${this.$v.name.$params.minLength.min} characters long`)
      !this.$v.name.required && errors.push('Name is required')
      return errors
    },
    checkboxErrors () {
      const errors = []
      if (!this.$v.checkbox.$dirty) return errors
      !this.$v.checkbox.checked && errors.push('You must agree to continue!')
      return errors
    }
  }

}

</script>
